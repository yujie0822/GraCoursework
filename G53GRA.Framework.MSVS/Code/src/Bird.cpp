...
Bird::Bird(const float& width, const float& height, const std::string& filename)
...
void Bird::Update(const double& deltaTime) {
	...
	{
		pos[0] += scale[0] * dx;
		if (pos[0] > scale[0] * 60.f || pos[0] < -scale[0] * 60.f)
			dx = -dx;

		if (time > 0.1)
		{
			spriteFrame = spriteFrame > 20 ? 0 : spriteFrame + 1; // shift frame up by 1 (wrap at 21)
